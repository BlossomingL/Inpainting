...
def random_bbox(config):
    ...
    maxt = img_height - config.VERTICAL_MARGIN - config.HEIGHT  # max top
    maxl = img_width - config.HORIZONTAL_MARGIN - config.WIDTH  # max left
    t = tf.random_uniform(
        [], minval=config.VERTICAL_MARGIN, maxval=maxt, dtype=tf.int32)  # 32~96
    l = tf.random_uniform(
        [], minval=config.HORIZONTAL_MARGIN, maxval=maxl, dtype=tf.int32)  # 32~96
