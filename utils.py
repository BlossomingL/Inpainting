...
def gen_input_mask(shape, hole_size, count, hole_area=None, max_holes=1):
    """
    * inputs:
    ...
    * returns:
    ...
    """
    mask = np.zeros(shape)
    bsize, mask_h, mask_w, _ = mask.shape
    for i in range(bsize):
        ...
        for _ in range(n_holes):
            ...
            mask[i, offset_y: offset_y + hole_h, offset_x: offset_x + hole_w, :] = 255.0
    cv2.imwrite('masks/' + str(count) + '.png', mask[0])
    return mask